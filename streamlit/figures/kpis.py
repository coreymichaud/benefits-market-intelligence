...
def calculate_kpis(data, year=2023):
    ...
    year_core = core[core["FORM_YEAR"] == year].copy()
    ...
    year_valid = valid_base[valid_base["FORM_YEAR"] == year].copy()
    ...
    year_plans = plans[plans["FORM_YEAR"] == year].copy()

    participants = year_plans["TOT_PARTCP_BOY_CNT"].sum(min_count=1)
    ...
    year_suspect = data["valid"][
        (data["valid"]["FORM_YEAR"] == year) & (data["valid"]["suspect"])
    ].copy()

    flagged_contracts = len(year_suspect)

    total_contracts = len(data["valid"][data["valid"]["FORM_YEAR"] == year])
    ...
    p90_plan_rate = (
        year_plan_rates["commission_rate"].quantile(0.90)
        if not year_plan_rates.empty
        else np.nan
    )
    ...
    return {
        ...
        "p90_plan_rate_pct": p90_plan_rate * 100 if pd.notna(p90_plan_rate) else np.nan,
    }
